...
void loop() {
  ...
  if(count%(8*speed_cube) == 0){
    if(Serial.available()>0){
      ...
      Serial.println(serialListener_temp);
      if((serialListener_temp==48) || (serialListener_temp==49) || (serialListener_temp==50) || (serialListener_temp==51)){
        serialListener=serialListener_temp;
        Serial.println(serialListener);
      }
    }
    int a=int(serialListener)-48;
    initialise(a);
  }
